...
const { selectedYear, notify } = useKoperasiStore();
const months = [
...
const filteredMonths = computed(() =>
  months.filter((row) => String(row[0]).endsWith(String(selectedYear.value))),
);
</script>
<template>
  <div class="page-stack">
    <PageHeader title="Rekonsiliasi"
      ><template #actions
        ><button
      ...
      ></PageHeader
    >
    <section v-if="filteredMonths.length === 0" class="panel year-empty-state">
      <CalendarX2 :size="36" />
      <strong>Belum ada data rekonsiliasi untuk {{ selectedYear }}</strong>
      <p>Pilih tahun lain untuk melihat status dan riwayat rekonsiliasi.</p>
    </section>
    <template v-else>
      ...
      <section class="panel table-panel">
        ...
        <div class="data-table-wrap">
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="row in filteredMonths" :key="String(row[0])">
                <td>
                  <strong>{{ row[0] }}</strong>
                </td>
